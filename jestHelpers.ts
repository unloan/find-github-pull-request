...
/**
 * Taken from @see: https://docs.github.com/en/rest/reference/commits#list-pull-requests-associated-with-a-commit
 */
...
/** NOTE: This factory isn't perfect, we do not update all references in the fixture, eg. the PR number is referenced dozens of times. */
export const pullRequestFactory = (number: number, overrides?: object) => ({
  ...pullRequestFixture,
  // TODO: We could recursively update all places where a PR number exists to match…only replacing what's used.
  html_url: pullRequestFixture.html_url.replace(`/${pullRequestFixture.number}`, `/${number}`),
  number,
